...
*/
	...
	public function findCourse($pid)
	{

		global $pdo;
		
		$sqlPoint=$pdo->prepare('SELECT `courseId` FROM `point` WHERE `pid` = :pid;');
	...
	public function list($courseId,$name)
	{
		...
		if($nowRoleId==1)
		{
			$sqlPoint=$pdo->prepare('SELECT * FROM `point` WHERE `name` LIKE :name AND `courseId`=:courseId AND `visibility` <> -1 ;');
			$sqlPoint->bindValue(':name','%'.urlencode($name).'%',PDO::PARAM_STR);
			$sqlPoint->bindValue(':courseId',(int)$courseId,PDO::PARAM_INT);
			$sqlPoint->execute();
		...
		{
			$response=[];
			if(Course::findOwner($courseId)==$nowId)
			{
				$sqlPoint=$pdo->prepare('SELECT * FROM `point` WHERE `name` LIKE :name AND `courseId`=:courseId AND visibility <>-1;');
				$sqlPoint->bindValue(':name','%'.urlencode($name).'%',PDO::PARAM_STR);
				$sqlPoint->bindValue(':courseId',(int)$courseId,PDO::PARAM_INT);
	...
	public function modify($pid,$importance,$name,$content,$courseId,$order,$visibility)
	{
		...
		if(checkAuthority())
		{
			...
			$nowRoleId=$nowUser[0]['roleId'];
			$nowId=$nowUser[0]['uid'];	
			if($nowRoleId==2&&($nowId!=Course::findOwner($courseId)))
				return false;
		}
		else 
			return false;
		...
		$sqlPoint=$pdo->prepare('UPDATE `point` SET `updateTime`=:updateTime,`importance`=:importance, `name` = :name , `content` = :content,`order`=:order, `visibility` = :visibility WHERE `pid` = :pid ;');
